...
public class Solv04 {
	public static void main(String[] args) {
		//전화번호는 xxx-xxxx-xxxx 의 형식으로 입력받을것이고
		//이 번호를 문자열로 저장하고
		//문자열 중에 - 의 갯수만큼 반복해서
		//번호를 나누고 배열에 저장하면 된다.
		
		//1.먼저 입력 받아서 변수에 저장하고
		...
		// ==> 전화번호를 분리를 하게 되면 '-'의 갯수보다 1개가 더많은 문자열로 분리가 된다.
		String[] arr = new String[cnt + 1];
		//4. 배열에 문자열을 짤라서 입력하고
		// 잘려진 문자열을 저장할 변수(작업할 변수)를 만든다.
		// 처음에는 전체 문자열이 모두 저장이 되어야 될것이다.
		String tmp = phone;
		int idx = 0;
		for(int i = 0 ; i < cnt ; i++) {
			//'-'의 인덱스를 알아낸다.
	//idx = tmp.indexOf('-');
			...
			// 이제 뒷 번호를 다시 짤라서 tmp 에다 저장해놓고
			// 다시 위의 명령들을 반복하면된다.
			tmp = tmp.substring(idx+1);
		}
		arr[cnt] = tmp;
		//5. 출력하고
		JOptionPane.showMessageDialog(null,"입력한번호: " + phone + 
									"\n분리해 놓은 배열 :" + Arrays.deepToString(arr));
	}
}
